...
# AIFCA Python code Version 0.9.16 Documentation at https://aipython.org
...
class NN(Learner):
    ...
    def learn(self, epochs=None, batch_size=32, num_iter = 100, report_each=10):
        ...
        num_iter is the number of iterations over the batches
             - overrides epochs if provided
        report_each means print errors after each multiple of that number of batches
        """
        self.batch_size = min(batch_size, len(self.dataset.train)) # don't have batches bigger than training size
        if num_iter is None:
             num_iter = (epochs * len(self.dataset.train)) // self.batch_size
        self.report_each = report_each
        if not self.printed_heading:
            self.display(0,"batch\tTraining\tTraining\tValidation\tValidation")
            self.display(0,"\tAcccuracy\tLog loss\tAcccuracy\tLog loss")
            self.printed_heading = True
            self.trace()
        for i in range(num_iter):
            batch = random.sample(self.dataset.train, self.batch_size)
            for e in batch:
                # compute all outputs
    ...
    def trace(self):
        ...
        self.display(0,self.bn,"\t",
            "\t\t".join("{:.4f}".format(
                    self.dataset.evaluate_dataset(dataset, self.predictor, criterion))
                            for dataset in [self.dataset.train, self.dataset.valid]
...
# nn3.learn(epochs=None, batch_size=100, num_iter = 1000, report_each=100)
...
#nn3do.learn(epochs=None, batch_size=100, num_iter = 1000, report_each=100)
...
class NN_from_arch(NN):
    def __init__(self, data, arch, optimizer=SGD, **hyperparms):
        ...
        hyperparms is the list of parameters of the optimizer 
        ...
        self.add_layer(Linear_complete_layer(self,output_size, final_layer=10))
...
def plot_algs(archs=[[3]], opts=[SGD],lrs=[0.1, 0.01,0.001,0.0001],
                  data=data, criterion=crit,  num_steps=1000):
    args = []
    for arch in archs:
        for opt in opts:
            for lr in lrs:
                args.append((arch,opt,{'lr':lr}))
    plot_algs_opts(args,data, criterion,  num_steps)

def plot_algs_opts(args, data=data, criterion=crit,  num_steps=1000):
    ...
    for (arch, opt, hyperparms) in args:
        ...
        plot_steps(learner = nn, data = data, criterion=crit, num_steps=num_steps,
                       log_scale=False, legend_label=str(nn))

## first select good learning rates for each optimizer.
# plot_algs(archs=[[3]], opts=[SGD],lrs=[0.1, 0.01,0.001,0.0001])
# plot_algs(archs=[[3]], opts=[Momentum],lrs=[0.1, 0.01,0.001,0.0001])
# plot_algs(archs=[[3]], opts=[RMS_Prop],lrs=[0.1, 0.01,0.001,0.0001])

## If they have the same best learning rate, compare the optimizers:
# plot_algs(archs=[[3]], opts=[SGD,Momentum,RMS_Prop],lrs=[0.01])

## With different learning rates, compare the optimizer using:
# plot_algs_opts(args=[([3],SGD,{'lr':0.01}), ([3],Momentum,{'lr':0.1}), ([3],RMS_Prop,{'lr':0.001})])
...
# start_time = time.perf_counter();nn_mnist.learn(epochs=10, batch_size=128);end_time = time.perf_counter();print("Time:", end_time - start_time,"seconds")  #1 epoch
...
# Plot learning:
# plot_algs(archs=[[32],[32,8]], opts=[RMS_Prop], lrs=[0.01], data=data_mnist, num_steps=100)
# plot_algs(archs=[[8],[8,8,8],[8,8,8,8,8,8,8]], opts=[RMS_Prop], lrs=[0.01], data=data_mnist, num_steps=100)
...
if __name__ == "__main__":
    ...
    NN_from_arch(data, arch=[3]).evaluate()
