...
class ActionEvent(object):
    ...
    # !! assumo che arrivi un MyCard.Player
    def get_legal_actions(state, player):
        ...
        Args:
            ...
            player: Il giocatore corrente (MyCard.Player).
        ...
        #correggere anche la codifica delle azioni tramite il BitMasking
        ...
        for card in play_onto.get_players_card(player):
            legal_actions.append(DiscardAction(card[1]))

        # Aggiunge l'azione di chiusura del gioco
        card = csp_resolver.can_end_game_csp(player)
        if card != None:
            legal_actions.append(CloseGameAction(card[1]))
...
class CloseGameAction(ActionEvent):
    def __init__(self, card_id):
        super().__init__(action_id=close_game_action_id + card_id)
        self.card_id = card_id
