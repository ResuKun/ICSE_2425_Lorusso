...
import Player.player_onto_modifier as pom
...
class BurracoGame:
    ...
    def step(self, action: ActionEvent):
        ...
        if isinstance(action, PickUpCardAction):
            pom.pesca_carta(current_player)
        elif isinstance(action, PickUpDiscardAction):
            pom.pesca_scarti(current_player)

        elif isinstance(action, OpenMeldAction):
            pom.apre_scala(current_player, action.meld)
        elif isinstance(action, OpenTrisAction):
            pom.apre_tris(current_player, action.tris)
        elif isinstance(action, UpdateMeldAction):
            pom.aggiunge_carte_scala(current_player, action.update)
        elif isinstance(action, UpdateTrisAction):
            pom.aggiunge_carte_tris(current_player, action.update)

        elif isinstance(action, DiscardAction):
            pom.scarta_carta(current_player, action.card_id)

        elif isinstance(action, CloseGameAction):
            pom.aggiunge_carte_tris(current_player, action.card)


        elif isinstance(action, ScoreFirstPlayerAction):
            pom.get_player_score(current_player)
        elif isinstance(action, ScoreSecondPlayerAction):
            pom.get_player_score(next_player)
