...
class BurracoRound:
    ...
    def pick_up_discard(self, action: PickUpDiscardAction):
        ...
        self.log.info(f"-----------------------------------------NUOVO TURNO---------------------------------")
        self.log.info(f"{current_player.player1.nomeGiocatore} ")
        self.log.info(f"---- carta pescata ----> {[carta.name for carta in current_player.player1.playerHand.mazzo[-len_cards:]]}")
        self.log.info(f"---- CARTE IN MANO: ")
        self.log.info(f"{[card.name for card in current_player.player1.playerHand.mazzo]}")
        self.log.info(f"-------------------------------------------------------------------------------------")
    ...
    def open_meld(self, action: OpenMeldAction):
        current_player = self.players[self.current_player_id]
        cards = [onto_access_util.get_card_from_id(card_id) for card_id in action.cards_ids]
        ...
        self.log.info(f"+++--------------------------------------OPEN MELD---------------------------------")
        self.log.info(f"{current_player.player1.nomeGiocatore} ----> {[carta.name for carta in cards]}")
