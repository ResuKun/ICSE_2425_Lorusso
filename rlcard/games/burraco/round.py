...
class BurracoRound:
    ...
    def pick_up_card(self, action: PickUpCardAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [pick_up_card] --> {current_player.player1.playerHand.mazzo[-1].name}")
    ...
    def pick_up_discard(self, action: PickUpDiscardAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [pick_up_discard] --> {current_player.player1.playerHand.mazzo[-len_cards].name}")
    ...
    def open_meld(self, action: OpenMeldAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [Apro Scala] --> {cards}")
    ...
    def open_tris(self, action: OpenTrisAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [Apro Tris] --> {cards}")
    ...
    def update_meld(self, action: UpdateMeldAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [UP Canasta] --> {onto_access_util.get_meld_from_id(meld_id).semeScala} --> {onto_access_util.get_card_from_id(action.card_id).name}")
    ...
    def update_tris(self, action: UpdateTrisAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [UP Tris] --> {onto_access_util.get_tris_from_id(tris_id).trisValue} -->{onto_access_util.get_card_from_id(action.card_id).name}")
    ...
    def discard(self, action: DiscardAction):
        ...
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [Scarto] --> {onto_access_util.get_card_from_id(action.card_id).name}")

    def close_game(self,action: CloseGameAction):
        current_player = self.players[self.current_player_id]
        player_onto_util.chiudi_gioco(current_player.player1,action.card_id)
        self.is_over = True
        self.log.info(f"{current_player.player1.nomeGiocatore} --> [Chiudo] --> {onto_access_util.get_card_from_id(action.card_id).name}")
