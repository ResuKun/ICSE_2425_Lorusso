...
# update_tris_action_id = 11598 - 11627
# update_meld_action_id = 11628 - 11677

#Creazione della mappatura delle azioni
#capire come tenerlo in memoria senza doverlo leggere da file
@lru_cache(maxsize=1)
def create_mapping():
...
def _generate_melds(matte_list, onto):
    ...
    for seme in all_semes:
        ...
        for index in range(1,12):
            ...
            # SCALE CON MATTA
            for remaining_cards in itertools.product(copies_R2, copies_R3):
                for matta in matte_list: # 12 Matte
                    meld = (matta, remaining_cards[0], remaining_cards[1])
...
def _generate_update_tris():
    tris_udpate_arr = []
    # A 3/7 - 8/13(K)
    tris_card_values = [15, 5, 10]

    # 0 matta, 1 carta pulita
    for is_clean in range(0, 2):
        # per ogni "tipologia" di Tris
        for value in tris_card_values:
            # considero da il totale di carte finale ( da 4 a 8+)
            for index in range(4, 9):
                tris_type = (is_clean, index, value)
...
def _generate_update_meld():
    ...
    for is_clean in range(0, 2):
        ...
        for value in meld_card_values:
            # considero da il totale di carte finale ( da 4 a 8+)
            for index in range(4, 9):
                meld_type = (is_clean, index, value)
