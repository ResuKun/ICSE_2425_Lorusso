...
def salva_ontologia():
    ...
    print(f"UPDATE::  Ontologia aggiornata in CARD_CARD_GAME_FILE")

def salva_ontologia_init_game():
    onto.save(file = OntologyResource.INIT_GAME.value, format = "rdfxml")
    print(f"UPDATE::  Ontologia aggiornata in CARD_INIT_GAME_FILE")

def salva_ontologia_update_game():
    print(list(onto.individuals()))
    for g in onto.Monte.instances():
        print(g)
    for g in onto.Game.instances():
        print(g)
    onto.save(file = OntologyResource.UPDATED_GAME.value, format = "rdfxml")
    print(f"UPDATE::  Ontologia aggiornata in CARD_UPDATED_GAME_FILE")

def create_update_file():
     file = open(OntologyResource.CARD.value, "r")
     content = file.read()
     file.close()
     content = content.replace("http://www.semanticweb.org/les/ontologies/2025/Cards","http://www.semanticweb.org/les/ontologies/2025/Updated_Game_Ontology")
     print(content)
